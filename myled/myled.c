...
MODULE_AUTHOR("Ryuichi Ueda & Shun Kaneko");
...
static int pin[number] = {25, 16, 21, 20};
...
static ssize_t led_write( struct file* filp, const char* buf, size_t count, loff_t* pos) 
{
	...
	if(c  == '1') {
		gpio_base[10] = 1 << pin[0];
		msleep(500);
		gpio_base[7] = 1 << pin[0];
	}
	else if(c == '2') {
		gpio_base[10] = 1 << pin[1];
		msleep(500);
		gpio_base[7] = 1 << pin[1];
	}
	else if(c == '3') {
		gpio_base[10] = 1 << pin[2];
		msleep(500);
		gpio_base[7] = 1 << pin[2];
	}
	else if(c == '4') {
		gpio_base[10] = 1 << pin[3];
		msleep(500);
		gpio_base[7] = 1 << pin[3];
	}
	else if(c == '0') {
